...
if(file.exists(params$ebsafile)) {
  ...
  data <- readRDS('../data/ebsa_SEA_16.rds')
}
...
overview <- list(Records = nrow(occ),
     ...
     'Contributing institutes'= length(unique(na.omit(occ$institutionCode))),
     Taxa = nrow(taxa),
...
```{r peryearpertaxa}
ggplot(occ %>% filter(!is.na(year) & year >= 1950), aes(year, fill=taxonomicgroup)) + 
  geom_bar() +
...
countbyyear <- occ %>% 
  ...
  group_by(year) %>% 
  summarise(count=n()) %>%
  right_join(., data_frame(year=1950:as.integer(format(Sys.Date(), "%Y"))))
countbyyear$count[is.na(countbyyear$count)] <- 0
...
# Sampling events (1950 - present)

```{r samplingevents}
eventsbyyear <- occ %>% 
  filter(!is.na(eventDate) & !is.na(as.numeric(year))) %>% 
  select(year, eventDate, resource_id, depth, decimalLongitude, decimalLatitude) %>% 
  distinct() %>% 
  group_by(year) %>% 
  summarise(count=n()) %>%
  right_join(., data_frame(year=1950:as.integer(format(Sys.Date(), "%Y"))))
eventsbyyear$count[is.na(eventsbyyear$count)] <- 0
...
statusimportance <- function(status) {
  ...
  si <- unlist(lapply(importance, function(x) ifelse(is.null(x), 1, x)))
  if(is.null(si)) {
    si <- integer(0) 
  }
  si
...
```{r newestsp}
newesttable <- occ %>% filter(!is.na(year)) %>%
...
before1950table <- occ %>% filter(!is.na(year)) %>%
  group_by(valid_id, taxonomicgroup) %>% 
  summarise(max_year=max(year)) %>% ungroup() %>%
  left_join(taxa) %>% 
